...
class UserShow extends React.Component {
  ...
  componentDidMount() {
    const { fetchImages, fetchUsers, user, fetchLikes, match } = this.props;
    fetchImages();
    fetchUsers();
  ...
  handleCoverPicSubmit() {
    const { user, updateUserProfilePic } = this.props;
  ...
  userProfileLikesImpressions() {
    ...
    let imagesIdArray = [];
    let imagesLikesArray = [];
    let count = 0;
    images.filter(image => image.uploaderId === user.id).forEach(image => imagesIdArray.push(image.id));
    imagesIdArray.forEach(id => (imagesLikesArray.push(likes.filter(like => like.imageId === id))));
    imagesLikesArray.forEach(image => count += image.length);

    return currentUser.id === user.id ? (
      <div className="user-profile-likes-impressions-container">
        <div className="user-profile-likes">
          <p><span className="user-profile-likes-number">{count} Photo Likes</span></p>
        </div>
        <div className="user-profile-impressions">
          <p><span className="user-profile-impressions-number">{count*imagesLikesArray.length+12} Photo Impressions</span></p>
      ...
      <div className="other-user-profile-likes-impressions-container">
        ...
        <div className="other-user-profile-likes">
          <p><span className="other-user-profile-likes-number">{count} Photo Likes</span></p>
        </div>
        <div className="other-user-profile-impressions">
          <p><span className="other-user-profile-impressions-number">{count*imagesLikesArray.length+12} Photo Impressions</span></p>
  ...
  userProfileOtherInfo(navBar) {
    ...
    const userImages = images.filter((image) => image.uploaderId === Number(match.params.userId))
    switch (navBar) {
      case 'photos':
        return (
...
//     fetchImages();
//     fetchUsers();
//     user ? setState({id: user.id}) : setState({id: Number(match.params.userId)});
//     fetchLikes();
//     window.scrollTo(0, 0);
//   }, []);
...
//   const handleCoverPicSubmit = () => {
//     const formData = new FormData();
//     if (state.coverpic) {
//       formData.append('user[coverpic]', state.coverpic);
//       formData.append('user[id]', state.id);
//     }
//     updateUserProfilePic(formData, user.id);
//   }

//   const handleCoverPicFile = (e) => {
//     e.preventDefault();
//     const file = e.target.files[0];
//     const fileReader = new FileReader();
...
//           <svg xmlns="http://www.w3.org/2000/svg" width="28" height="28" viewBox="0 0 24 24"><path d="M5 4h-3v-1h3v1zm10.93 0l.812 1.219c.743 1.115 1.987 1.781 3.328 1.781h1.93v13h-20v-13h3.93c1.341 0 2.585-.666 3.328-1.781l.812-1.219h5.86zm1.07-2h-8l-1.406 2.109c-.371.557-.995.891-1.664.891h-5.93v17h24v-17h-3.93c-.669 0-1.293-.334-1.664-.891l-1.406-2.109zm-11 8c0-.552-.447-1-1-1s-1 .448-1 1 .447 1 1 1 1-.448 1-1zm7 0c1.654 0 3 1.346 3 3s-1.346 3-3 3-3-1.346-3-3 1.346-3 3-3zm0-2c-2.761 0-5 2.239-5 5s2.239 5 5 5 5-2.239 5-5-2.239-5-5-5z"/></svg>
//         </label>
//         <input type="file" id="user-cover-picture-input" onInput={handleCoverPicFile}/>
//       </div>
//     )
//   }
...
//     if (state.profilepic) {
//       formData.append('user[profilepic]', state.profilepic);
//       formData.append('user[id]', state.id);
//     }
...
//           <svg xmlns="http://www.w3.org/2000/svg" width="28" height="28" viewBox="0 0 24 24"><path d="M5 4h-3v-1h3v1zm10.93 0l.812 1.219c.743 1.115 1.987 1.781 3.328 1.781h1.93v13h-20v-13h3.93c1.341 0 2.585-.666 3.328-1.781l.812-1.219h5.86zm1.07-2h-8l-1.406 2.109c-.371.557-.995.891-1.664.891h-5.93v17h24v-17h-3.93c-.669 0-1.293-.334-1.664-.891l-1.406-2.109zm-11 8c0-.552-.447-1-1-1s-1 .448-1 1 .447 1 1 1 1-.448 1-1zm7 0c1.654 0 3 1.346 3 3s-1.346 3-3 3-3-1.346-3-3 1.346-3 3-3zm0-2c-2.761 0-5 2.239-5 5s2.239 5 5 5 5-2.239 5-5-2.239-5-5-5z"/></svg>
//         </label>
//         <input type="file" id="user-profile-picture-input" onInput={handleProfilePicFile}/>
//       </div>
//     )
//   }
...
//   const userProfileLikesImpressions = () => {
//     let imagesIdArray = [];
//     let imagesLikesArray = [];
//     let count = 0;
//     images.filter(image => image.uploaderId === user.id).forEach(image => imagesIdArray.push(image.id));
...
//   const userProfileOtherInfo = (navBar) => {
//     const userImages = images.filter((image) => image.uploaderId === Number(match.params.userId))
//     switch (navBar) {
//       case 'photos':
//         return (
